Shader "SLFramework/RangeIndicator/RI_Circle"
{
    Properties 
	{
		...
		_Power ("Power", float) = 5								// Power越大，波越细
		_Strength ("Strength", float) = 1						// Strength越大，波越粗
		...
		Pass 
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				else
				{
					...
					// 当前角度是否在规定的扇形里
					range = 1 - step(smoothstep(_AttackAngle * 0.5, 0, angle) 
					- smoothstep(0, -_AttackAngle * 0.5, angle), 0);		
				}

				// 将uv投影到纹理（alpha非0即1）中
				// 用于限制区域
