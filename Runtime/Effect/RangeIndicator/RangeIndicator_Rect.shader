Shader "SLFramework/RangeIndicator/RI_Rect"
{
    Properties 
	{
		...
		Pass 
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				*/
				...
				// 条形波
				float dis = (uv.y - uv.x) + 1.0 + _Time.y * _ScanSpeed;
				dis *= _ScanInterval;
				dis = dis - floor(dis);

				float lineWave = step(dis, _ScanSize);


				float alpha = (borderRect + lineWave) * fullMask;
